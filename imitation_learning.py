...
#         log_probs = torch.log(self.policy(states).gather(1, actions)) # 最大化专家action的概率
...
class GAIL:
    def __init__(self, agent, state_dim, action_dim, hidden_dim, lr_d):
        ...
        # 生成器, 模仿者策略和环境交互生成数据, reward取决于判别器, 目标是模范者生成的数据, 判别器预估为专家数据(p越小)
        self.agent = agent
    ...
    def learn(self, expert_s, expert_a, agent_s, agent_a, next_s, dones):
        ...
        # 训练判别器，分别预估专家数据和agent数据为agent数据的概率
        ...
        # 判别器预估为agent的概率作为reward, 
        # agent_prob越大说明生成的数据很容易被判别器识别，需要降低reward; 
        # agent_prob越小说明生成的数据被判别器识别为专家数据，需要增大reward;
...
with tqdm(total=n_episode, desc="进度条") as pbar:
    for i in range(n_episode):
        ...
        while not done:
            ...
            state = next_state
            # 这里的reward是生成器和环境交互产生的, 可以从侧面说明生成器的效果
            episode_return += reward
        return_list.append(episode_return)
